...
describe("filterOpportunities", () => {
  ...
  it("parses only known keys, collecting repeated values", () => {
    expect(parseFilters({ stage: ["SALES", "PROJECT"], status: "", foo: "x", view: "table" }))
      .toEqual({ stage: ["SALES", "PROJECT"], status: [], accountable: [], account: [], q: undefined });
  });
  it("filters by stage", () => {
  ...
  it("treats several values of one filter as OR", () => {
    expect(filterOpportunities(rows, { stage: ["SALES", "PROJECT"] }).map((r) => r.number)).toEqual([2, 3]);
  });
